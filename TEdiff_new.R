...
count_column = 2
experiment_formula = "sample:replicant:condition"
sample_names = "adult:1:adult,adult:2:adult,egg:1:egg,egg:2:egg,juv:1:juv,juv:2:juv,met:1:met,met:2:met"
...
res <- results(dds)
write.csv(res, file = paste0('~/ncbi', "/nbinom", "~", variable_names[1], ".csv"))
...
for(i in seq(from=TE_step, to=length(TE_row), by = TE_step))
{
    ...
    pdf(paste0(outdir, "/heatmap_", old_i, "-", i, ".pdf") , height=20,width=30)
    print(heatmap.2(assay(TE_vsd)[select,], col = hmcol, Rowv = FALSE, Colv = FALSE, scale="none", dendrogram="none", trace="none", margin=c(10, 6)))
    x = dev.off()
    png(paste0(outdir, "/heatmap_", old_i, "-", i, ".png") , height=800,width=900, units='px')
    print(heatmap.2(assay(TE_vsd)[select,], col = hmcol, Rowv = FALSE, Colv = FALSE, scale="none", dendrogram="none", trace="none", margin=c(10, 6)))
    x = dev.off()
